...
def knn(config, train_data_df, test_data_df, pc=False, scal=False, lasso=False, minmax=False, n_neighbors=5):
    ...
    results_df = pd.DataFrame({
        'Configuration': [
            f'config={config}, pca={pc}, scal={scal}, lasso={lasso}, minmax={minmax}, n_neighbors={n_neighbors}'],
        'Accuracy': [accuracy],
        'Precision': [precision],
        'Recall': [recall],
        'F1 Score': [f1]
    })
    results_df.to_csv('results_knn1.csv', mode='a', index=False)
